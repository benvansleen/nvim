...
local function toggle_fstring()
    ...
    do
        local node
        do
            local mod_6_auto = require("nfnl.module").autoload("lib.treesitter")
            node = mod_6_auto["nearest-parent-of-type"]("string")
        end
        if not node then
            ...
            do
                ...
                srow, scol, _ecol, _erow = mod_6_auto.get_vim_range({ node:range() })
            end
