...
(let [cats (require :nixCatsUtils)
      lze (require :lze)
      old_ft_fallback (lze.h.lsp.get_ft_fallback)]
  ...
  (lze.load [(tb :nvim-lspconfig
             ...
             (tb :lazydev.nvim
                 {:for_cat :neonixdev
                  ...
                  :ft [:lua]
                  :after (fn [_]
                           (let [lazydev (require :lazydev)]
                             (lazydev.setup {:library {:words [:nixCats]
             ...
             (tb :rnix {:enabled (not cats.isNixCats) :lsp {:filetypes [:nix]}})
             (tb :nil_ls {:enabled (not cats.isNixCats)
                          :lsp {:filetypes [:nix]}})
             (tb :nixd
