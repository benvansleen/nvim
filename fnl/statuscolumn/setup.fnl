...
(fn statuscolumn.defaults []
  "%l%s")

(fn statuscolumn.init []
  (statuscolumn.highlights)
  (or (table.concat [(statuscolumn.center-buffer)
                     (statuscolumn.folds)
...
(fn update-screen-width []
  ...
  ;; force redraw of statuscolumn
  (set vim.wo.statuscolumn vim.wo.statuscolumn))
...
(config (g {my_center_buffer true
        ...
        (autocmd {[:BufLeave :BufWinLeave :WinLeave :WinResized :VimResized] {:callback update-screen-width}}))
