...
(fn force-statuscolumn-redraw []
  (when (not= vim.bo.filetype :dashboard)
    (cfg (wo {statuscolumn (statuscolumn.activate)}))))
...
(cfg (g {my_center_buffer true
     ...
     (autocmd {[:BufWinEnter :BufWinLeave] {: group
                                            :callback force-statuscolumn-redraw}
               [:WinNew :WinEnter :WinResized :VimResized] {: group
                                                            :callback (fn []
                                                                        (update-screen-width)
                                                                        (force-statuscolumn-redraw))}}))
