...
do
	...
	do
		local _ = { nil, nil, nil, nil, nil, nil, nil, nil, nil }
	end
	...
	do
		local _ = {
			...
			vim.keymap.set("n", "<leader>wtf", _3_),
			vim.keymap.set("n", "<leader>q", _4_),
		}
	end
	do
	...
	do
		local _ = {
			...
			vim.api.nvim_create_autocmd(
				...
				{ pattern = "*", group = numbertoggle, callback = _7_, nested = false, once = false }
			),
			vim.api.nvim_create_autocmd(
				{ "BufLeave", "FocusLost", "InsertEnter", "CmdlineEnter", "WinLeave" },
				{ pattern = "*", group = numbertoggle, callback = _9_, nested = false, once = false }
			),
		}
	end
end
...
if nixCats("format") then
	return require("myLuaConf.format")
else
	return nil
end
